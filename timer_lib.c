 ...
 */
void timer2_set_external_clock() { ASSR |= (1 << AS2); }

/*
 * When using Timer2, the internal clock can be configured for use
 * (recommended method).
 */
void timer2_set_internal_clock() { ASSR &= ~(1 << AS2); }
...
void timer_setup(uint8_t timer, uint8_t waveGenMode, int prescale,
  ...
  if (timer == 0) {
    ...
    switch (prescale) {
      case 0:
        clockSelect = 0;
        break;  // no clocking, frozen timer.
      case 1:
        clockSelect = 1;
        break;
      case 8:
      ...
      default:
        clockSelect = 7;
        break;  // external clock source rising edge.
    }
  } else {
  ...
  uint8_t tccrNa = (compareOutModeA << 6) | (compareOutModeB << 4) |
                   (compareOutModeC << 2) | (waveGenMode & 3);
  uint8_t tccrNb =
      (captureMode << 6) | ((waveGenMode & 0xC) << 1) | clockSelect;
