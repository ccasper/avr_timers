...
#define UNLOCK_INTERRUPTS SREG = __old_SREG

#define LOCK_TIMER2_COMPA TIMSK2 &= ~(1 << OCIE2A)
#define UNLOCK_TIMER2_COMPA TIMSK2 |= (1 << OCIE2A)

static inline void onboard_led_enable(void) { setpin_out(DDRB, 7); }
static inline void onboard_led_on(void) { setpin(PORTB, 7); }
static inline void onboard_led_off(void) { clearpin(PORTB, 7); }
...
int main(void) {
  ...
  timer_setup(2 /* timer2 */, 2 /* CTC */, 64 /* prescale */,
              0 /* disconnect A output pin */, 0 /* disconnect B output pin */,
              0 /* disconnect C output pin */, 0 /* captureMode */);
